...
- **`uttera-stt-hotcold`** (*this repository*) — Whisper hot/cold
  worker pool with optional LibreTranslate post-processing for
  arbitrary target languages. Ideal for consumer GPUs and home-lab
  deployments.
- **`uttera-stt-vllm`** — Whisper-v3-turbo via vLLM's native speech
  path, single-process continuous batching. Ideal for cloud,
  multi-tenant, large-VRAM GPUs.
- **`uttera-tts-hotcold`** — TTS sibling of this repository with
  pluggable backends (Coqui XTTS-v2, VoxCPM2).
- **`uttera-tts-vllm`** — TTS sibling of `uttera-stt-vllm`, built on
...
protocol, `X-Route` response header semantics, and sit behind the
same [echo-gatekeeper](https://github.com/uttera) for tier gating,
